    ...
    const handleUpdate = async (field: "status" | "priority", value: string) => {
      ...
      try {
        const updatedBug = { 
          ...
          fixed_by: field === "status" && value === "fixed" ? currentUser.id : undefined, // Set fixed_by if status is fixed
        };
            ...
            <div className="space-y-2 py-3 border-t border-border">
              <div className="flex flex-col space-y-1">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium">{bugState.reporter_name || bug.reporter_name}</span>
                </div>
                ...
                {bugState.status === "fixed" && (bugState.fixed_by_name || bug.fixed_by_name) && (
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Fixed By:</span>
                    <span className="font-medium">{bugState.fixed_by_name || bug.fixed_by_name}</span>
                  </div>
                )}
              </div>
            </div>
