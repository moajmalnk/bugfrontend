...
const FILTER_TABS: ComplianceFilterTab[] = ['all', 'completed', 'pending', 'not_started'];
  ...
  const tabCounts = useMemo(() => {
    ...
    for (const tab of FILTER_TABS) {
      counts[tab] = overviewItems.filter((item) =>
        matchesComplianceFilter(item, tab, roleKey)
      ).length;
    }
    return counts;
  }, [overviewItems, roleKey]);

  const filteredItems = useMemo(() => {
    const query = searchTerm.trim().toLowerCase();

    let items = overviewItems.filter((item) =>
      matchesComplianceFilter(item, activeTab, roleKey)
    );
