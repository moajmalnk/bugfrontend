...
function fcmResolveNotificationPayload(payload) {
...
fcmMessaging.onBackgroundMessage(function (payload) {
  const resolved = fcmResolveNotificationPayload(payload);
  const data = payload && payload.data ? payload.data : {};
...
function fcmOpenTargetUrl(targetUrl) {
  const absoluteUrl = fcmToAbsoluteUrl(targetUrl);
...
const APP_CACHE_VERSION = '1.0.27';
...
function getCacheStrategy(request) {
  ...
  if (url.pathname.match(/\.(js|css|png|jpg|jpeg|gif|svg|ico|woff|woff2|mp4|webm)$/)) {
    return 'cacheFirst';
  }
